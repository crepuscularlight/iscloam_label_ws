...
// #endif
        int label_temp = (int)filtered_pointcloud->points[i].label;
        if (isc.at<unsigned char>(ring_id, sector_id) < label_temp)
        isc.at<unsigned char>(ring_id, sector_id) = label_temp;
    }
